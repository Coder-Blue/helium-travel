  ...
  previewImage.addEventListener("load", async () => {
    ...
    try {
      ...
      await $fetch(url, {
        ...
        body: formData,
        headers: {
          "x-amz-checksum-algorithm": "SHA-256",
        },
      });
      ...
      if (e instanceof FetchError) {
        errorMessage.value = (e as FetchError).statusMessage || "Lỗi bất ngờ đã xảy ra.";
      }
      else if (e instanceof Error) {
        errorMessage.value = (e as Error).message;
      }
      else {
        errorMessage.value = "Lỗi bất ngờ đã xảy ra.";
      }
    }
...
<template>
  <div>
    <h2 class="mb-2">
      Quản lí ảnh của "{{ locationLog?.name || "Đang tải..." }}"
    </h2>
    <div class="flex">
